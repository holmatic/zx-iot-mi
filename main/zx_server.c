...
{
    char buf[FILFB_SIZE];
    zx_string_to_ascii(file_first_bytes,file_name_len,buf);
    ...
    while(true){
		if(pdTRUE ==  xQueueReceive( msg_queue, &evt, 10 / portTICK_RATE_MS ) ) {
            ...
            if(evt.evt_type==ZXSG_HIGH){
                ...
                for(size_t i=0;i<sizeof(ldrfile);i++){
                    stzx_send_cmd(STZX_FILE_DATA,ldrfile[i]);                    
                }
                ...
                if(evt.addr<FILFB_SIZE){
                    ...
                    if(file_name_len==0 && (evt.data&0x80) ){
                    ...
                    if(evt.addr==1){
                        if(file_first_bytes[0]==ZX_SAVE_TAG_LOADER_RESPONSE){
                            ...
                            for(size_t i=0;i<sizeof(menufile);i++){
                                d=menufile[i];
                                if(i>=16444-16393 && i<16477-16393) d=ldrfile[i+1]; // assume name is length 1
                                stzx_send_cmd(STZX_FILE_DATA,d);                    
                            }
                            stzx_send_cmd(STZX_FILE_END,0);
