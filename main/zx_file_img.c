...
void zxfimg_create(zxfimg_prog_t prog_type) {
	...
	if(!memimg) memimg=malloc(16384);
	for(i=0;i<16507-img_offs;i++){
		memimg[i]=src[i];
	}
	sz=mem_img_size();
	printf("File size %d bytes",sz);
	for(;i<sz;i++){
		memimg[i]=src[i];
	}
}
